...
test.describe('Time Stats View Features', () => {
    let appPage: AppPage;
    ...
    // Helper function to create a task and add some time to it
    async function createTaskWithTime(title: string) {
        await appPage.openAddTaskModal();
        await taskPage.createTask({
            title: title,
            description: 'Task for Time Stats testing'
        });

        // Start and stop timer to add time history
        await timerPage.startTimer(title);
        await appPage.page.waitForTimeout(2000); // 2 seconds
        await timerPage.pauseTimer(title);
    }
